...
func main() {
	...
	fmt.Println(headerStyle.Render("Tool Use Demo"))
	fmt.Println()
	...
	for {
		...
		for _, block := range resp.Content {
			...
			case anthropic.ToolUseBlock:
				...
				handler, ok := handlers[b.Name]
				if !ok {
					logger.Fatal("Unknown tool", "name", b.Name)
				}

				resultJSON, err := handler(b.Input)
				if err != nil {
					logger.Fatal("Tool execution failed", "tool", b.Name, "err", err)
				}

				logger.Info("Tool executed", "tool", b.Name)
				fmt.Println(dimStyle.Render(fmt.Sprintf("  result: %s", resultJSON)))
				fmt.Println()
